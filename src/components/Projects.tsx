...
const colorMap: Record<Color, { badge: string; text: string; border: string; shadow: string }> = {
  green:  { badge: 'badge-green', text: 'text-neon-green',  border: 'border-neon-green',  shadow: '4px 4px 0px #00ff87' },
  cyan:   { badge: 'badge',       text: 'text-neon-cyan',   border: 'border-neon-cyan',   shadow: '4px 4px 0px #00e5ff' },
  pink:   { badge: 'badge-pink',  text: 'text-neon-pink',   border: 'border-neon-pink',   shadow: '4px 4px 0px #ff00ff' },
  yellow: { badge: 'badge-yellow',text: 'text-neon-yellow', border: 'border-neon-yellow', shadow: '4px 4px 0px #ffd700' },
}
  ...
  return (
    <section id="projects" className="py-20 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        {loading && (
          <p className="font-pixel text-[9px] text-pixel-dim animate-pulse">&gt; Loading projects...</p>
        )}
        ...
        {!loading && projects.length === 0 && (
          <div className="pixel-card p-10 text-center border-neon-green/20">
            ...
            <p className="font-retro text-xl text-pixel-dim">Projects are coming soon — check back later!</p>
          </div>
        )}
          ...
          {projects.map((proj, i) => {
            ...
            return (
              <motion.div
                ...
                className={`pixel-card flex flex-col group border-bg-border hover:${c.border} transition-all`}
                style={{ ['--hover-shadow' as string]: c.shadow }}
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  {proj.tech.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {proj.tech.map((t) => (
                        <span key={t} className={`badge ${c.badge} text-[10px]`}>{t}</span>
                      ))}
                    </div>
                  )}
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    {proj.links.github && (
                      <a href={proj.links.github} target="_blank" rel="noopener noreferrer"
                        className={`text-pixel-dim hover:${c.text} transition-colors`} aria-label="GitHub">
                        <GithubIcon size={15} />
                      </a>
                    )}
                    {proj.links.itchio && (
                      <a href={proj.links.itchio} target="_blank" rel="noopener noreferrer"
                        className="text-pixel-dim hover:text-neon-pink transition-colors" aria-label="itch.io">
                        <ItchioIcon size={15} />
                      </a>
                    )}
                    {proj.links.live && (
                      <a href={proj.links.live} target="_blank" rel="noopener noreferrer"
                        className={`text-pixel-dim hover:${c.text} transition-colors`} aria-label="Live site">
                        <ExternalLink size={14} />
                      </a>
                    )}
                    {proj.links.demo && (
                      <a href={proj.links.demo} target="_blank" rel="noopener noreferrer"
                        className={`font-pixel text-[8px] ${c.text} hover:underline`}>
                        demo
                      </a>
                    )}
                    {proj.links.devpost && (
                      <a href={proj.links.devpost} target="_blank" rel="noopener noreferrer"
                        className="font-pixel text-[8px] text-neon-cyan hover:underline">
                        devpost
                      </a>
                    )}
                    {proj.links.paper && (
                      <a href={proj.links.paper} target="_blank" rel="noopener noreferrer"
                        className="font-pixel text-[8px] text-neon-yellow hover:underline">
                        paper
                      </a>
                    )}
                  </div>
